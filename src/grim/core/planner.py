...
def build_plan(path: str, network: bool = True, deep: bool = False) -> dict:
    ...
    add("malware_scan", "second",
        "Signature + heuristic malware scan (webshells, ELF, IoC hashes; ClamAV/YARA if installed).")
    add("check_live", "optional",
        "Authorized live checks (headers, cookies, TLS, exposed paths) - requires a scope file.")
    if deep:
